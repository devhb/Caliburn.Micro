...
namespace Caliburn.Micro {
    ...
    public class DefaultCloseStrategy<T> : ICloseStrategy<T> where T : class {
        ...
        /// </summary>
        /// <param name="toClose">Items that are requesting close.</param>
        public async Task<CloseStrategyResult<T>> Execute(IEnumerable<T> toClose)
        {
            ...
            foreach (var item in toClose)
            {
                ...
                {
                    var canClose = await guard.CanClose();
                    if (canClose)
                    {
                        closables.Add(item);
                    }
            ...
            return new CloseStrategyResult<T>(result, closeConductedItemsWhenConductorCannotClose ? closables : Enumerable.Empty<T>());
        }
